...
static int trailers_list(struct mg_connection *conn) {
    ...
    const char *sql =
        "SELECT id, unit_number, vin, type, status, current_km "
        "FROM trailers ORDER BY id DESC;";
...
static int trailers_create(struct mg_connection *conn) {
    ...
    if (!cJSON_IsString(type) || !is_valid_type(type->valuestring)) {
        cJSON_Delete(root);
        send_json(conn, 400, "{\"error\":\"type_required_reefer_dry_van_flatbed\"}");
        return 400;
    }
    ...
    const char *sql =
        "INSERT INTO trailers(unit_number, vin, type, status, current_km) "
        "VALUES(?, ?, ?, ?, ?);";
    ...
    if (cJSON_IsString(status)) sqlite3_bind_text(stmt, 4, status->valuestring, -1, SQLITE_TRANSIENT);
    else sqlite3_bind_text(stmt, 4, "active", -1, SQLITE_TRANSIENT);

    if (cJSON_IsNumber(current_km)) sqlite3_bind_double(stmt, 5, current_km->valuedouble);
    else sqlite3_bind_double(stmt, 5, 0.0);
