...
static int trucks_list(struct mg_connection *conn) {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        int year = sqlite3_column_int(stmt, 3); if (year) cJSON_AddNumberToObject(o, "year", year);
        const unsigned char *make = sqlite3_column_text(stmt, 4); if (make) cJSON_AddStringToObject(o, "make", (const char *)make);
        const unsigned char *model = sqlite3_column_text(stmt, 5); if (model) cJSON_AddStringToObject(o, "model", (const char *)model);
        const unsigned char *engine = sqlite3_column_text(stmt, 6); if (engine) cJSON_AddStringToObject(o, "engine", (const char *)engine);
        const unsigned char *status = sqlite3_column_text(stmt, 7); if (status) cJSON_AddStringToObject(o, "status", (const char *)status);
...
static int trucks_create(struct mg_connection *conn) {
    sqlite3 *db = db_handle();
    ...
    if (cJSON_IsNumber(year)) sqlite3_bind_int(stmt, 3, year->valueint); else sqlite3_bind_null(stmt, 3);
    if (cJSON_IsString(make)) sqlite3_bind_text(stmt, 4, make->valuestring, -1, SQLITE_TRANSIENT); else sqlite3_bind_null(stmt, 4);
    if (cJSON_IsString(model)) sqlite3_bind_text(stmt, 5, model->valuestring, -1, SQLITE_TRANSIENT); else sqlite3_bind_null(stmt, 5);
    if (cJSON_IsString(engine)) sqlite3_bind_text(stmt, 6, engine->valuestring, -1, SQLITE_TRANSIENT); else sqlite3_bind_null(stmt, 6);
    if (cJSON_IsString(status)) sqlite3_bind_text(stmt, 7, status->valuestring, -1, SQLITE_TRANSIENT); else sqlite3_bind_text(stmt, 7, "active", -1, SQLITE_TRANSIENT);
    if (cJSON_IsNumber(current_km)) sqlite3_bind_double(stmt, 8, current_km->valuedouble); else sqlite3_bind_double(stmt, 8, 0.0);
...
int handle_api_trucks(struct mg_connection *conn, void *cbdata) {
    (void)cbdata;
    const struct mg_request_info *ri = mg_get_request_info(conn);
    if (strcmp(ri->request_method, "GET") == 0) return trucks_list(conn);
    if (strcmp(ri->request_method, "POST") == 0) return trucks_create(conn);
